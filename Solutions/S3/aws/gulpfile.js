...
gulp.task("assets", () => {
  return gulp
    .src([
      ...
      "./src/client/wakka.png",
      "./src/client/wakka-logo.svg",
    ])
    .pipe(gulp.dest("./dist"));
});
...
// Start express
gulp.task("express", () => {
  const tsc = exec("nodemon --watch dist/tsc/server dist/tsc/server/server.js");
  tsc.stdout.on("data", (data) => console.log(data));
  tsc.stderr.on("data", (data) => console.error(data));
});
...
gulp.task(
  "build",
  gulp.series("clean", "scss", "index", "assets", "tsc", "bundle")
);
...
// Heroku copy html files
gulp.task("heroku-copy-html", () => {
  return gulp.src(["./dist/**/*.html"]).pipe(gulp.dest("./deploy/dist"));
});
